...
public:
    ...
    ParamServer()
    {
        ...
        nh.param<float>(PROJECT_NAME + "/ang_res_y", ang_res_y, 1.0);
        nh.param<int>(PROJECT_NAME + "/lidar_type", lidar_type, 1);
        nh.param<std::string>(PROJECT_NAME + "/timeField", timeField, "time");
        nh.param<int>(PROJECT_NAME + "/downsampleRate", downsampleRate, 1);
        nh.param<float>(PROJECT_NAME + "/lidarMaxRange", lidarMaxRange, 50);
        nh.param<float>(PROJECT_NAME + "/lidarMinRange", lidarMinRange, 0.5);
        nh.param<int>(PROJECT_NAME + "/feature_enable", feature_enable, 1);
        nh.param<int>(PROJECT_NAME + "/remove_noise", remove_noise, 0);

        nh.param<int>(PROJECT_NAME + "/min_cluster_size", min_cluster_size, 10);
        nh.param<int>(PROJECT_NAME + "/segment_valid_point_num", segment_valid_point_num, 5);
        nh.param<int>(PROJECT_NAME + "/segment_valid_line_num", segment_valid_line_num, 3);
    ...
    // {
    //     cout << "imu in: " << imu_in.orientation.w << " " << imu_in.orientation.x << " " << imu_in.orientation.y << " " <<  imu_in.orientation.z << endl;
